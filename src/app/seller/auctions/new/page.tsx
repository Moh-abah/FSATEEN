...
import { useSellerProducts, useCreateAuction } from '@/hooks';
...
export default function NewAuctionPage() {
  ...
  const { data: products, isLoading: productsLoading } = useSellerProducts(1, 50);
  ...
  const onSubmit = (data: AuctionFormData) => {
    createAuction(data, {
      ...
      onError: () => {
        toast({
          title: 'حدث خطأ أثناء إنشاء المزاد',
          variant: 'destructive',
        });
      },
    });
          ...
          {activeProducts.length === 0 ? (
            <Card className="border border-[var(--border)] bg-[var(--surface)]">
              <CardContent className="py-8 text-center">
                ...
                <h3 className="text-lg font-medium text-[var(--text)] mb-2">لا توجد منتجات متاحة</h3>
                ...
                <Button asChild className="bg-[var(--primary)] hover:bg-[var(--primaryDark)] text-[var(--textInverse)]">
                  <a href="/seller/products/new">إضافة منتج جديد</a>
                </Button>
              </CardContent>
            </Card>
          ) : (
